...
//        return true;
     ...
     *
     * 对于C(n,m)组合数公式的推导：
     * 挑选第1个，可以从剩下的n中选择n种，可以从剩下的m个位置中选择一个位置进行摆放，即m种摆放
     * 挑选第2个，可以从剩下的n中选择(n-1)种，可以从剩下的(m-1)个位置中选择一个位置进行摆放，即(m-1)种摆放
     * ...
     * 挑选第m个，可以从剩下的n中选择(n-m+1)种，可以从剩下的1个位置中选择一个位置进行摆放，即1种摆放
     *
     * 首先考虑有序的排列组合，即
     * n*(n-1)*...*(n-m+1) = n!/(n-m)!
